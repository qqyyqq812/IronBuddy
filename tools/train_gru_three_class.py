...
def _synthesize_non_standard(bad_arr):
    """把 MIA bad rep 映射到推理/模拟器一致的 non_standard 特征空间.

    MIA bad CSV 的 Symmetry_Score 很低, 但板端推理时 symmetry 是 1.0 占位值。
    若直接训练原始 bad, 模型会依赖上线时不可用的 symmetry 信号。这里保留
    bad 的角度/相位形态, 但把 target/comp 显式压到"发力不足"区间。
    """
    out = bad_arr.copy()
    n = out.shape[0]
    phase = out[:, 6].astype(np.float32)
    t_base = np.random.uniform(8.0, 26.0, size=n).astype(np.float32)
    c_base = np.random.uniform(5.0, 22.0, size=n).astype(np.float32)
    out[:, 3] = np.clip(t_base + 6.0 * phase + np.random.normal(0, 2, n), 3, 35)
    out[:, 4] = np.clip(c_base + 5.0 * phase + np.random.normal(0, 2, n), 2, 30)
    out[:, 5] = 1.0
    return out
...
def _normalize_inplace(features):
    ...
    # 推理侧 symmetry 是占位值 1.0; 训练也固定到 1.0, 避免模型依赖离线标签泄漏.
    features[:, 5] = 1.0
    # Phase 本身 0-1 不处理
    return features
...
def build_dataset(mia_dir, n_comp_per_golden=N_COMP_PER_GOLDEN):
    ...
    for fp in bad_files:
        ...
        if len(rep) < 10:
            continue
        arr = _synthesize_non_standard(_rep_to_array(rep))
...
def _make_case(label, mia_dir=None):
    ...
    elif label == "non_standard":
        arr = _synthesize_non_standard(arr)
