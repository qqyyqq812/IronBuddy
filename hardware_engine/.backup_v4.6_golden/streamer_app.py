...
def feishu_smart_push():
    ...
    try:
        ...
        bot_reply = ds_resp["choices"][0]["message"]["content"]
    except Exception as e:
        return Response(json.dumps({"ok": False, "error": "DeepSeek 推理失败: " + str(e)}),
                        mimetype='application/json', status=500)
    ...
    try:
        ...
        if msg_resp.get("code") == 0:
            return Response(json.dumps({"ok": True, "type_triggered": push_type, "plan": bot_reply}), mimetype='application/json')
        else:
