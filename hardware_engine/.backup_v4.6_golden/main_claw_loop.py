...
async def main():
    ...
    try:
        while True:
            ...
            if should_trigger:
                # 立刻清零疲劳值，避免锁未释放/生成失败时疲劳值卡住 1500 导致反复触发
                try:
                    fsm.total_fatigue_volume = 0
                    fsm.sync_to_frontend()
                except Exception as _e:
                    logging.warning("[fatigue] 清零失败: %s", _e)
                _ds_lock[0] = True
                _last_deepseek_time = time.time()
                ...
                if connected:
                    ...
                    fl = _fatigue_limit[0]
                    # V4.5: 疲劳值汇报必须使用真实上限（用户可改为 1900 等），禁止 LLM 默认 1500
                    fatigue_str = f"达到本组目标 {fl} 分" if current_fatigue >= fl - 10 else f"{current_fatigue:.1f}/{fl}"

                    short_prompt = (
                        f"你是一名客观专业的健身教练。\n"
                        f"本组数据: 疲劳 {fatigue_str}, 标准深蹲 {good_count} 次, 违规半蹲 {failed_count} 次, 合格率 {rate_pct}%\n"
                        f"**疲劳目标阈值是 {fl}**（当前用户设定值，不是固定 1500）。点评中若提及疲劳分数，必须使用 {fl} 作为分母或目标。\n"
