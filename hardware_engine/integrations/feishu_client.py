...
class FeishuClient(object):
    ...
    def _send_message(self, msg_type, content_obj, chat_id=None):
        ...
        if self.dry_run:
            ...
            _LOG.info(
                ...
                msg_type, masked_target, preview,
            )
            return {"ok": True, "dry_run": True, "msg_type": msg_type,
                    "chat_id_masked": masked_target}
    ...
    @staticmethod
    def build_training_card(title, body_text, stats=None, push_type="summary",
                            degraded=False, footer=""):
        """统一训练卡片：手动推送、总结、提醒、计划都走 interactive。

        stats: dict with optional exercise/good/failed/comp/fatigue/fatigue_limit.
        """
        stats = stats or {}
        template_map = {
            "summary": "green",
        ...
        template = template_map.get(push_type, "blue")
        if degraded:
            template = "yellow"

        exercise = stats.get("exercise") or "squat"
        if exercise == "bicep_curl":
            ...
            exercise_label = "深蹲"
        fatigue_limit = stats.get("fatigue_limit", 1500) or 1500
        stats_md = (
            "**动作**：%s\n"
            "**标准**：%s 次\n"
            "**不标准**：%s 次\n"
        ...
        note_text = footer or ("IronBuddy · 降级卡片" if degraded else "IronBuddy")
        elements = [
            FeishuClient._md("**训练状态**\n" + stats_md),
            FeishuClient._hr(),
            FeishuClient._md("**教练建议**\n" + str(body_text or "（暂无内容）")),
            {
                "tag": "note",
                "elements": [
                    {"tag": "plain_text", "content": str(note_text)}
                ],
            },
        ]
        return {
            "config": {"wide_screen_mode": True},
            "header": FeishuClient._header(str(title or "IronBuddy 训练卡片"), template),
            "elements": elements,
        }
