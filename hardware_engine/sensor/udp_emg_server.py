...
try:
    from hardware_engine.sensor.mvc_calibration import (
        build_payload as _build_mvc_payload,
        load_mvc_values as _load_mvc_values,
        atomic_write_json as _write_mvc_json,
    )
except Exception:
    try:
        from mvc_calibration import (  # type: ignore
            build_payload as _build_mvc_payload,
            load_mvc_values as _load_mvc_values,
            atomic_write_json as _write_mvc_json,
        )
    except Exception:
        _build_mvc_payload = None
        _load_mvc_values = None
        _write_mvc_json = None
...
IS_CONNECTED = False
LAST_ALIVE_TS = 0
EMG_STREAM_RING_LIMIT = 1000
RAW_RING = deque(maxlen=EMG_STREAM_RING_LIMIT)
STREAM_RING = deque(maxlen=EMG_STREAM_RING_LIMIT)
RAW_DEBUG = {
    "ts": 0.0,
    "addr": "",
    "packet_count": 0,
...
if os.path.exists(_MVC_JSON):
    try:
        if _load_mvc_values is not None:
            _MVC_VALUES, _md = _load_mvc_values(_MVC_JSON)
        else:
...
def dsp_receiver_worker():
    global CURRENT_RMS_PCT, IS_CONNECTED, LAST_ALIVE_TS, RAW_DEBUG
    ...
    while True:
        try:
            ...
            raw0 = float(vals[0]) if len(vals) >= 1 else 0.0
            raw1 = float(vals[1]) if len(vals) >= 2 else (float(vals[0]) if vals else 0.0)
            RAW_RING.append([LAST_ALIVE_TS, raw0, raw1])

            # 对每个通道走 DSP
            filtered_vals = [0.0, 0.0]
            rms_vals = [0.0, 0.0]
            mvc_vals = [
                float(_MVC_VALUES.get('target', 400.0)),
                float(_MVC_VALUES.get('comp', 400.0)),
            ]
            ...
            if len(vals) == 1:
                ...
                filtered_vals[1] = filtered_vals[0]
                rms_vals[1] = rms_vals[0]
            ...
            RAW_DEBUG = {
                ...
                "raw_values": vals[:2],
                "filtered": filtered_vals,
                "rms": rms_vals,
                "mvc": mvc_vals,
                "pct": list(CURRENT_RMS_PCT[:2]),
                "domain": {
                    "target": list(_DOMAIN_CALIB.get('target', (1.0, 0.0))),
                    "comp": list(_DOMAIN_CALIB.get('comp', (1.0, 0.0))),
                },
            }
            STREAM_RING.append([
                LAST_ALIVE_TS,
                raw0,
                raw1,
                float(filtered_vals[0]),
                float(filtered_vals[1]),
                float(rms_vals[0]),
                float(rms_vals[1]),
                float(CURRENT_RMS_PCT[0]),
                float(CURRENT_RMS_PCT[1]),
                int(pkt_count),
            ])
...
def _check_mvc_request():
    ...
    try:
        if _build_mvc_payload is not None and _write_mvc_json is not None:
            _payload = _build_mvc_payload(
                peak_target,
                peak_comp,
                user_id='runtime',
                exercise='current',
                source='udp_emg_server',
                ts=now,
            )
            _payload['window_sec'] = _MVC_CAL_WINDOW_SEC
            _write_mvc_json(_MVC_JSON, _payload)
        else:
            tmp = _MVC_JSON + '.tmp'
    ...
    try:
        with open(_MVC_RESULT_PATH, 'w') as _rf:
            if _build_mvc_payload is not None:
                json.dump(_build_mvc_payload(
                    peak_target,
                    peak_comp,
                    user_id='runtime',
                    exercise='current',
                    source='udp_emg_server_result',
                    ts=now,
                ), _rf)
            else:
                json.dump({
...
def io_dumper_worker():
    ...
    while True:
        ...
        if not IS_CONNECTED:
            # No recent packet. Keep the last atomic heartbeat for a short grace
            # window so bursty ESP32 UDP does not make the UI flicker offline.
            if LAST_ALIVE_TS <= 0 or now - LAST_ALIVE_TS > 3.0:
                try:
        ...
        out = {
            ...
            "warnings": warnings,
            "exercise": exercise,
            "target_rms": float(RAW_DEBUG.get("rms", [0.0, 0.0])[0] or 0.0),
            "comp_rms": float(RAW_DEBUG.get("rms", [0.0, 0.0])[1] or 0.0),
            "target_mvc": float(RAW_DEBUG.get("mvc", [400.0, 400.0])[0] or 400.0),
            "comp_mvc": float(RAW_DEBUG.get("mvc", [400.0, 400.0])[1] or 400.0),
            "target_pct": float(target_pct or 0.0),
            "comp_pct": float(comp_pct or 0.0),
            "connected": True,
            "signal_mode": "real_udp",
            "valid": True,
        }
        debug_out = dict(RAW_DEBUG)
        debug_out.update({
            "exercise": exercise,
            "connected": True,
        ...
        raw_wave_out = {
            "ts": RAW_DEBUG.get("ts", 0.0),
            "connected": True,
            "packet_count": RAW_DEBUG.get("packet_count", 0),
            "sample_count": len(RAW_RING),
            "channels": ["raw_adc_ch0", "raw_adc_ch1"],
            "samples": list(RAW_RING),
        }
        stream_out = {
            "ts": RAW_DEBUG.get("ts", 0.0),
            "connected": True,
            "packet_count": RAW_DEBUG.get("packet_count", 0),
            "sample_count": len(STREAM_RING),
            "columns": [
                "ts", "raw0", "raw1", "filtered0", "filtered1",
                "rms0", "rms1", "pct0", "pct1", "packet_count"
            ],
            "channels": ["target_ch0", "comp_ch1"],
            "samples": list(STREAM_RING),
        }
        ...
        try:
            ...
            with open('/dev/shm/emg_debug_snapshot.json.tmp', 'w') as f:
                json.dump(debug_out, f)
            os.rename('/dev/shm/emg_debug_snapshot.json.tmp', '/dev/shm/emg_debug_snapshot.json')

            with open('/dev/shm/emg_raw_waveform.json.tmp', 'w') as f:
                json.dump(raw_wave_out, f)
            os.rename('/dev/shm/emg_raw_waveform.json.tmp', '/dev/shm/emg_raw_waveform.json')

            with open('/dev/shm/emg_stream_buffer.json.tmp', 'w') as f:
                json.dump(stream_out, f, separators=(',', ':'))
            os.rename('/dev/shm/emg_stream_buffer.json.tmp', '/dev/shm/emg_stream_buffer.json')
