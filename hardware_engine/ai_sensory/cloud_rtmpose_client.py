...
─────────────────────
  CLOUD_RTMPOSE_URL   – full URL to the /infer endpoint
                        default: http://127.0.0.1:6006/infer via SSH tunnel
...
MJPEG_IDLE_TIMEOUT_S = float(os.environ.get("MJPEG_IDLE_TIMEOUT_S", "8.0"))
            ...
            try:
                ...
                while True:
                    ...
                    if jpeg is not None and seq != last_seq:
                        ...
                        last_seq = seq
                        last_emit = time.time()
                    elif time.time() - last_emit > MJPEG_IDLE_TIMEOUT_S:
                        break
                    time.sleep(0.02)
...
PREVIEW_SCALE = float(os.environ.get("PREVIEW_SCALE", "0.65"))
...
def _select_camera_device():
    import glob as _glob
    by_id_paths = sorted(_glob.glob("/dev/v4l/by-id/*video-index0"))
    if by_id_paths:
        return by_id_paths[0]
    by_id_paths = sorted(_glob.glob("/dev/v4l/by-id/*index0"))
    if by_id_paths:
        return by_id_paths[0]
    for dev in ("/dev/video5", "/dev/video0"):
...
def _open_camera():
    video_dev = _select_camera_device()
    print("[CloudClient] Camera device: {}".format(video_dev))
    ...
    try:
        print("[CloudClient] Camera request: {}x{} {} @ {}fps".format(
            CAMERA_WIDTH, CAMERA_HEIGHT, CAMERA_FOURCC, TARGET_FPS))
    except Exception:
        pass
    return cap, video_dev
...
def _write_result_jpg(frame: np.ndarray, kpts: list) -> None:
    ...
    if 0.1 < PREVIEW_SCALE < 1.0:
        drawn = cv2.resize(drawn, (0, 0), fx=PREVIEW_SCALE, fy=PREVIEW_SCALE,
...
def main():
    ...
    _cloud_status_phase = ["unknown"]
    _last_vision_mode_mtime = [0.0]

    def _set_cloud_status(phase, detail="", backend="cloud"):
        # type: (str, str, str) -> None
        _cloud_status_phase[0] = phase
        _write_cloud_status(phase, detail, backend)

    def _reset_inference_slots():
        # type: () -> None
        with _infer_lock:
    ...
    def _apply_vision_mode(new_mode, reason):
        # type: (str, str) -> bool
        nonlocal active_mode
        if new_mode == active_mode:
            return False
        print("[CloudClient] Vision mode switched: {} -> {} ({})".format(
            active_mode, new_mode, reason))
        active_mode = new_mode
        _reset_inference_slots()
        if active_mode == "local":
            _set_cloud_status("ready", "local backend ({})".format(reason), "local")
        else:
            _set_cloud_status("connecting", "switching to cloud ({})".format(reason), "cloud")
        return True

    # Initial status reflects the starting mode
    if active_mode == "local":
        _set_cloud_status("ready", "local backend at startup", "local")
    else:
        _set_cloud_status("connecting", "cloud worker starting", "cloud")
        ...
        while _infer_alive[0]:
            ...
            try:
                ...
                if resp.status_code == 200:
                    payload = resp.json()
                    if active_mode == "cloud" and not _first_cloud_ok[0]:
                        _first_cloud_ok[0] = True
                        _set_cloud_status("ready", "first response ok", "cloud")
                    ...
                    if active_mode == "cloud" and _consecutive_cloud_fail[0] >= 3 and _cloud_status_phase[0] != "failed":
                        _set_cloud_status(
                            "failed",
    ...
    try:
        while True:
            ...
            if simulate_mode:
                ...
                if not ret:
                    failed_reads += 1
                    if failed_reads >= 15 and time.time() - last_reopen_try > 2.0:
                        last_reopen_try = time.time()
                        try:
                            cap.release()
                        except Exception:
                            pass
                        ...
                        simulate_mode = not cap.isOpened()
                        failed_reads = 0
                        print("[CloudClient] Camera reopened: {} opened={}".format(
                            video_dev, not simulate_mode))
                    time.sleep(0.01)
                    continue
                failed_reads = 0
                orig_h, orig_w = frame.shape[:2]
            ...
            # V7.38: also consume the new mode immediately. Waiting for frame_idx
            # % 30 made UI hot-switch look broken for 1-2 seconds and could leave
            # the frontend staring at a stale "failed" status.
            if os.path.exists("/dev/shm/vision_reset.flag"):
                new_mode = _read_vision_mode(active_mode)
                _apply_vision_mode(new_mode, "reset flag")
                try:
                    os.remove("/dev/shm/vision_reset.flag")
                except Exception:
                    pass

            # ── Hot-switch vision mode (cheap mtime check every frame) ─────
            try:
                vm_mtime = os.path.getmtime(SHM_VISION_MODE)
            except Exception:
                vm_mtime = 0.0
            if vm_mtime and vm_mtime != _last_vision_mode_mtime[0]:
                _last_vision_mode_mtime[0] = vm_mtime
                new_mode = _read_vision_mode(active_mode)
                _apply_vision_mode(new_mode, "vision_mode mtime")
            ...
            # Shooting path: write every rendered frame by default. Override
            # JPEG_STRIDE only when deliberately trading preview FPS for CPU.
            _jpeg_stride = int(os.environ.get("JPEG_STRIDE", "1"))
