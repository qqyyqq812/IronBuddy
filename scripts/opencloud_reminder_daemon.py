#!/usr/bin/env python3
"""OpenClaw local/offboard IronBuddy reminder daemon.
...
INSIGHTS_PATH = os.path.join(RUNTIME_DIR, "opencloud_last_insights.json")
...
def _normalize_board_url(value):
    ...
    if value.startswith("http://") or value.startswith("https://"):
        ...
        value = scheme + "://" + host_port
    return value.rstrip("/")


def _board_url_from_ip(board_ip):
    return _normalize_board_url("%s:5000" % (board_ip or DEFAULT_BOARD_IP))


def resolve_board_url(board_url=None, board_ip=None):
    env_url = os.environ.get("IRONBUDDY_BOARD_URL")
    if board_url:
        return _normalize_board_url(board_url)
    if env_url:
        return _normalize_board_url(env_url)
    return _board_url_from_ip(board_ip or os.environ.get("IRONBUDDY_BOARD_IP") or DEFAULT_BOARD_IP)


DEFAULT_BOARD_URL = resolve_board_url()
...
def load_insights(board_url, board_online):
    """Fetch and cache OpenClaw insights, falling back to the last useful copy."""
    fetched = {}
    error = ""
    if board_online:
        fetched, error = _fetch_insights(board_url)
        if fetched and fetched.get("ok"):
            cached = dict(fetched)
            cached["_insights_source"] = "board"
            cached["_insights_ts"] = time.time()
            _atomic_json(INSIGHTS_PATH, cached)
            return cached, {
                "ok": True,
                "source": "board",
                "error": "",
                "cached_at": cached.get("_insights_ts"),
            }
    cached = _read_json(INSIGHTS_PATH)
    if cached and cached.get("ok"):
        cached = dict(cached)
        cached["_insights_source"] = "cached"
        return cached, {
            "ok": True,
            "source": "cached",
            "error": error,
...
def push_card(mode, snapshot, text, dry_run=True):
    ...
    card = FeishuClient.build_training_card(
        ...
        footer="IronBuddy OpenClaw · " + time.strftime("%Y-%m-%d %H:%M:%S"),
    )
...
def runtime_descriptor(board_url):
    return {
        "kind": "local_offboard",
        "location": "local/offboard",
        "offboard_runtime": True,
        "local_runtime": True,
        "host": socket.gethostname(),
        "pid": os.getpid(),
        "board_url": board_url,
        "process": "opencloud_reminder_daemon.py",
    }


def run_once(mode, board_url, dry_run=True):
    snapshot, board_online, error = load_snapshot(board_url)
    insights, insights_status = load_insights(board_url, board_online)
    ...
    runtime = runtime_descriptor(board_url)
    status = {
        ...
        "primary_runtime": runtime["kind"],
        "runtime_location": runtime["location"],
        "runtime": runtime,
        "insights_ok": bool(insights_status.get("ok")),
        "insights_source": insights_status.get("source"),
        "insights_error": insights_status.get("error"),
        "insights_cached_at": insights_status.get("cached_at"),
...
def _publish_schedule_only(board_url=None):
    ...
    payload = {
        ...
        "runtime": runtime_descriptor(resolve_board_url(board_url)),
    }
    try:
...
def loop(board_url, dry_run=True, interval=60):
    ...
    _publish_schedule_only(board_url)
    while True:
        now = time.localtime()
...
def main():
    parser = argparse.ArgumentParser(description="IronBuddy OpenClaw reminder daemon")
    parser.add_argument("--board-url", default=None)
    parser.add_argument("--board-ip", default=None, help="board IP shortcut, default 10.29.10.224")
    ...
    board_url = resolve_board_url(args.board_url, args.board_ip)
    dry_run = bool(args.dry_run or not args.send)
    if args.loop:
        loop(board_url, dry_run=dry_run, interval=args.interval)
        return
    mode = args.mode if args.mode != "auto" else "morning"
    status = run_once(mode, board_url, dry_run=dry_run)
