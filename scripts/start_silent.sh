...
TARGET="toybrick@10.29.10.224"
BOARD_KEY="$HOME/.ssh/id_rsa_toybrick"
CLOUD_KEY="$HOME/.ssh/id_cloud_autodl"
CLOUD_SSH="${CLOUD_SSH_HOST:-root@<set-CLOUD_SSH_HOST-env-var>}"
CLOUD_PORT=50203
CLOUD_RTMPOSE_URL="${CLOUD_RTMPOSE_URL:-http://127.0.0.1:6006/infer}"
...
timeout 300 rsync -az --checksum --info=progress2 --no-inc-recursive \
    ...
    echo "  ❌ rsync 失败或超时 (>300s). 检查: ssh toybrick@10.29.10.224 是否通畅"
    exit 1
}
...
echo "  Web:        http://10.29.10.224:5000/"
