...
#!/usr/bin/expect -f
set timeout 25
spawn ssh -N -L ${LPORT}:127.0.0.1:6006 -p ${PORT} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ControlMaster=no -o ControlPath=none -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes ${USER}@${HOST}
expect {
