...
def test_writes_connecting_on_cloud_switch():
    ...
    assert '_write_cloud_status("connecting"' in src or \
           "_write_cloud_status('connecting'" in src or \
           '_set_cloud_status("connecting"' in src


def test_writes_ready_on_first_frame():
    src = _read(CLIENT)
    assert '_write_cloud_status("ready"' in src or \
           "_write_cloud_status('ready'" in src or \
           '_set_cloud_status("ready"' in src
...
def test_hot_switch_uses_reset_flag_and_mtime_not_30_frame_poll():
    src = _read(CLIENT)
    assert "vision_reset.flag" in src
    assert "os.path.getmtime(SHM_VISION_MODE)" in src
    assert "frame_idx % 30" not in src
    assert "def _apply_vision_mode(" in src


def test_failed_status_can_recover_after_new_switch():
    src = _read(CLIENT)
    assert "_cloud_status_phase" in src
    assert "_set_cloud_status(\"connecting\"" in src
    assert "_set_cloud_status(\"ready\"" in src


def test_cloud_worker_does_not_overwrite_local_status():
    src = _read(CLIENT)
    assert 'if active_mode == "cloud" and not _first_cloud_ok[0]' in src
    assert 'if active_mode == "cloud" and _consecutive_cloud_fail[0]' in src
    assert 'if active_mode == "cloud":' in src
