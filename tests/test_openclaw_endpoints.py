...
def test_daemon_defaults_to_current_board_and_accepts_runtime_board_args():
    src = _read(DAEMON)
    assert "10.29.10.224" in src
    assert "IRONBUDDY_BOARD_URL" in src
    assert "IRONBUDDY_BOARD_IP" in src
    assert "--board-ip" in src
    assert "resolve_board_url" in src


def test_daemon_caches_insights_and_marks_local_offboard_runtime():
    src = _read(DAEMON)
    assert "opencloud_last_insights.json" in src
    assert "load_insights" in src
    assert "local/offboard" in src
    assert "local_offboard" in src
    assert "insights_source" in src
