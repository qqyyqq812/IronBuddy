...
def test_start_mvc_tool_is_ignored_by_voice_adapter(adapter_ns, Action):
    adapter_ns["_realize_action"](
...
def test_switch_exercise_curl_does_not_emit_mvc_followup_tip(adapter_ns, Action):
    """Lane D: MVC is UI-only and must not be suggested by voice."""
    speak = mock.MagicMock()
    adapter_ns["_realize_action"](
        Action("tool", u"好，切到弯举", "switch_exercise", {"action": "curl"}),
        speak_fn=speak,
    ...
    assert not any(u"MVC" in t or u"MCV" in t for t in spoken_texts)
