...
def test_manual_voice_fallback_routes_registered():
    src = _read(STREAMER)
    assert "MANUAL_VOICE_RECORD_PATH = \"/dev/shm/manual_voice_record.request\"" in src
    assert "MANUAL_VOICE_STOP_PATH = \"/dev/shm/manual_voice_stop.request\"" in src
    assert "MANUAL_VOICE_STATUS_PATH = \"/dev/shm/manual_voice_status.json\"" in src
    assert "@app.route('/api/voice_manual/start', methods=['POST'])" in src
    assert "@app.route('/api/voice_manual/stop', methods=['POST'])" in src
    assert "@app.route('/api/voice_manual/status')" in src
