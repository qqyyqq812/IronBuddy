...
def test_mvc_tool_is_not_exposed_to_voice_router():
    names = [tool["function"]["name"] for tool in TOOLS]
    assert "start_mvc_calibrate" not in names
    assert "start_mvc_calibrate" not in TOOL_ACK
...
def test_tools_spec_has_7_tools_after_mvc_voice_removal():
    assert len(TOOLS) == 7
