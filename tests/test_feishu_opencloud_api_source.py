...
def test_rag_delivery_endpoint_exposes_explicit_send_state():
    s = _read(STREAMER)
    body = s[s.find("def api_rag_delivery"):s.find("\n\ndef _read_voice_turn", s.find("def api_rag_delivery"))]
    assert '"send_mode": runtime.get("last_send_mode")' in body
    assert '"auto_send_enabled": _rag_auto_send_enabled()' in body
    assert "manual_send" in body
    assert "cooldown_s=(0 if manual_send else None)" in body
