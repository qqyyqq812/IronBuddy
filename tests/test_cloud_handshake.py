...
def test_switch_vision_resets_status_before_polling():
    src = _read(STREAMER)
    idx = src.find("def api_switch_vision(")
    assert idx != -1
    body = src[idx:idx + 2400]
    assert "_write_cloud_switch_status(mode)" in body
    assert "vision_reset.flag" in body


def test_flask_video_feed_fallback_not_capped_to_10fps():
    src = _read(STREAMER)
    idx = src.find("def video_feed(")
    assert idx != -1
    body = src[idx:idx + 1800]
    assert "IRONBUDDY_VIDEO_FEED_INTERVAL" in body
    assert "time.sleep(poll_interval)" in body
    assert "time.sleep(0.1)" not in body
