...
```bash
ssh -i ~/.ssh/id_rsa_toybrick toybrick@10.29.10.224 \
...
```bash
python tools/mvc_cli.py --user user_01 --duration 5 --reps 3 --board toybrick@10.29.10.224
```
...
for u in user_01 user_02 user_03; do
  for c in standard compensation bad_form; do
    python tools/upgrade_collect_7d_to_11d.py \
        --board-host toybrick@10.29.10.224 \
...
for c in standard compensation bad_form; do
  python tools/upgrade_collect_7d_to_11d.py --user user_04 --label $c \
      --board-host toybrick@10.29.10.224 --segment-len 60 --rep-len 2.0 \
