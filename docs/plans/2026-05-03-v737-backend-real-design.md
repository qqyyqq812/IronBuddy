...
```bash
# 1. 列追踪文件中含敏感关键字的
git ls-files | xargs grep -lE 'API_KEY|APP_SECRET|PASSWORD|WEBHOOK|seetacloud\.com|REDACTED_LEAKED_PWD' 2>/dev/null

# 2. 历史 commit 中含敏感 string 的
git log --all -p | grep -E 'REDACTED_LEAKED_PWD|seetacloud\.com.*password|FEISHU_APP_SECRET="|BAIDU_SECRET' | head -20
